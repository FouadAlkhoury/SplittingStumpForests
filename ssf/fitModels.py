...
def fitModels(roundSplit, iterations, dataset, XTrain, YTrain, XTest=None, YTest=None, createTest=False, model_dir='text',
              types=['RF', 'ET', 'DT'], 
			  forest_depths = [1,2],
              forest_sizes = [20,30]):
		...
		for i,size in enumerate(forest_sizes):
			for j,depth in enumerate(forest_depths):
				for k in range(iterations):
					acc = testModel(roundSplit, iterations, dataset, XTrain, YTrain, XTest, YTest, RandomForestClassifier(n_estimators=size, n_jobs=8, bootstrap= True, max_depth=depth), f"RF_{size}_{depth}", model_dir, size, depth)
					print(i)
					print(j)
