                    ...
                    for frequency in range(minThreshold, maxThreshold + 1):
                        for threshold in edge_thresholds:
                            ...
                            for model_type, model_class in learners.items():
                                ...
                                train_acc, learner_model = train_model_on_top(
                                    model_class(**learners_parameters[model_type]), fts_onehot, Y_train, scoring_function,
                                    model_type, graph_file)
                                ...
                                writeToReport(report_thresholds,
                                              str(size) + ',' + str(depth) + ',' + str(frequency) + ',' + str(
                                                  ...
                                                  np.round(f1_micro, 3)) + ',' + str(np.round(roc_auc, 3)) + ',' + str(
                                                  np.round((roc_auc - roc_auc_ref), 3)) + ',' + str(
